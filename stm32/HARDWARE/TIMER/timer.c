...
void TIM_Config()
{
   ...
   //设置该引脚为复用输出功能,输出TIM3 CH2的PWM脉冲波形	GPIOB.5
	...
	GPIO_InitStructure_1.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
...
void PWM_Init(TIM_TypeDef* timer, uint8_t time_channel, u16 arr, uint8_t duty)
{
	...
	TIM_TimeBaseStructure.TIM_ClockDivision = 0; //设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //TIM向上计数模式
	TIM_TimeBaseInit(timer, &TIM_TimeBaseStructure); //根据TIM_TimeBaseInitStruct中指定的参数初始化TIMx的时间基数单位
	
	//初始化TIM Channel PWM模式	 
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2; //选择定时器模式:TIM脉冲宽度调制模式1
 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_Pulse = (arr+1)*duty/100-1;
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
...
void PWM_SetDuty(TIM_TypeDef* timer, uint8_t time_channel, u16 duty)
{
	...
	{
		case 1:
			TIM_SetCompare1(timer,duty);
		break;
		case 2:
			TIM_SetCompare2(timer,duty);
		break;
		case 3:
			TIM_SetCompare3(timer,duty);
		break;
		case 4:
			TIM_SetCompare4(timer,duty);
		break;
	}
}
